...
private enum API_TYPE_FONT = "RenFont";

private int f_load(lua_State* L)
{
    const(char)* filename = luaL_checkstring(L, 1);
    float size = cast(float) luaL_checknumber(L, 2);
    RenFont** self = cast(RenFont**) lua_newuserdata(L, (RenFont*).sizeof);
...
private __gshared const luaL_Reg[] renderer_font_lib = [
    {"__gc", &f_gc},
    {"load", &f_load},
...
int luaopen_renderer_font(lua_State* L)
{
    ...
    luaL_setfuncs(L, renderer_font_lib.ptr, 0);
    lua_pushvalue(L, -1);
